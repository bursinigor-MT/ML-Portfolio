...
При выборе признаков основная цель состояла в том, чтобы максимально отразить поведение пользователей и характеристики контента, влияющие на вероятность просмотра видео более чем на 50%. Одним из ключевых факторов стал признак total_dur, поскольку именно он имел наибольший отклик у моделей. Интересно, что в этом датасете наблюдалась обратная от интуитивной зависимость — пользователи чаще досматривали длинные видео. Изначально мы предполагали, что чаще будут досматриваться более короткие видео, также были подозрения насчет особенно длинных роликов, которые, как оказалось, досматривались наполовину.  
Использование блединга моделей XGBoost и LightGBM позволило выявить сложные нелинейные зависимости, объединить сильные стороны обеих моделей и сократить количество ошибок. XGBoost оказался чувствительным к категориальным переменным, таким как длительность фильма, LightGBM откликался на частотно закодированные переменные и числовые признаки. 
